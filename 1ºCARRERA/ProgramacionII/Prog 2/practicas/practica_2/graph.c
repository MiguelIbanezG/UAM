...
int graph_findVertexById (const Graph *g, long id);
...
int graph_findVertexById (const Graph *g, long id) /*Esta función busca el vértice por Id*/
{
    int i;

    for (i = 0; i < graph_getNumberOfVertices(g); i++) 
    {
        if (id == vertex_getId(g->vertices[i])) /*Si encuentra un vértice con ese id, devolvera su posición*/
        {
...
int graph_getNumConnections_fromIndex (const Graph *g, int ix)
{
    ...
    return sizeof (graph_getConnectionsFromId(g, vertex_getId(g->vertices[ix])));
}
...
Status graph_depthSearch (Graph *g, long from_id, long to_id)
{
    ...
    if (!g || (from_id < 0) || (to_id < 0) || (graph_findVertexById(g, from_id) == -1) || (graph_findVertexById(g, to_id) == -1))
    {
        return ERROR;
    }
    ...
    vertex_setState (g->vertices[graph_findVertexById(g, from_id)], BLACK);

    stack_push (s, g->vertices[graph_findVertexById(g, from_id)]);


    while ((stack_isEmpty (s) == FALSE) && (st == OK) && (end == 0))
    {
       
        id0 = vertex_getId(stack_pop (s));
        ...
        for (i = 0; i < graph_getNumberOfConnectionsFromId(g, id0); i++)
        {
            if (vertex_getState (g->vertices[graph_getVertexpositionFromId (g, connections[i])]) == WHITE)
            {
                ...
                stack_push (s, g->vertices[graph_getVertexpositionFromId (g, connections[i])]);
            }
