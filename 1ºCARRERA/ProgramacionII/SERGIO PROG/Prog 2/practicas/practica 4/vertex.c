/*
*Título: Funciones vertex, práctica 1, programación 2
*Grupo: viernes 2113
...
Status vertex_setField(Vertex *v, char *key, char *value)
{
  ...
  {
    return vertex_setState(v, atoi(value));
  }
...
Vertex *vertex_init()
{
  ...
  v = (Vertex *)malloc(sizeof(Vertex)); /*Se hace la reserva de memoria*/

  if (v == NULL) /*Si falla dará error*/
  {
    return ERROR;
  }
...
int vertex_print(FILE *pf, const void *v)
{
  ...
  a = fprintf(pf, "[%ld, %s, %d, %d]", *(long *)vertex_getId(v), (char *)vertex_getTag(v), vertex_getState(v), vertex_getIndex(v));
