 ...
 *
 **/
typedef enum stat_Label{
    WHITE = 0, /*!< vertex not visited */
    BLACK = 1,  /*!< vertex visited */
    ERROR_VERTEX   /*!< not valid vertex */   
} Label;

typedef int (*P_vertex_cmp) (const void*, const void*);

typedef int (*P_vertex_cpy) (const void*, const void*);


typedef int (*P_vertex_print)(FILE *, const void *);
 ...
 *
 * @param v Vertex to set the index
 * @param index The index
 * 
 * @return OK if it is set correctly, ERROR if not. 
 */
 ...
 *
 * @param v Vertex to get the index
 *
 * @return The index. 
 */
