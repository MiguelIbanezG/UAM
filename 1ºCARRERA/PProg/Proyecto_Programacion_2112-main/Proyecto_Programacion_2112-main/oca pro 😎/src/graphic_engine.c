...
void graphic_engine_paint_game(Graphic_engine *ge, Game *game) /*Esta función pitna el tablero*/
{
  ...
  if ((id_act = game_get_player_location(game)) != NO_ID)
  {
    ...
    for (i = 0; i < 4; i++)
    {
      ...
      if (game_get_object_location(game, object_get_id(game_get_objects(game, i))) == id_back && space_get_light(game_get_space(game, id_back)) == TRUE)
      {
        if (i == 0)
          obj[i] = (char*)object_get_name (game_get_objects (game, i));
        else if (i == 1)
          obj[i] = (char*)object_get_name (game_get_objects (game, i));
        else if (i == 2)
          obj[i] = (char*)object_get_name (game_get_objects (game, i));
    ...
    if (id_back != NO_ID)
    {
      if (space_get_west(game_get_space(game, id_back)) != NO_ID) /*Si tiene una conexión desde el oeste hasta el este, se printeará*/
      {
        sprintf(str, "  %ld<--|             %2d|", link_getSpace1Id (game_get_link (game, space_get_west(game_get_space(game, id_back)))), (int)id_back);
      }
      else if (space_get_east(game_get_space(game, id_back)) != NO_ID) /*Si tiene una conexión desde el oeste hasta el este, se printeará*/
      {
        sprintf(str, "      |             %2d|-->%ld", (int)id_back, link_getSpace2Id (game_get_link (game, space_get_east(game_get_space(game, id_back)))));
      }
      else    /*Sino, no se printea nada*/
      {
    ...
    for (i = 0; i < 4; i++)
    {
      ...
      if (game_get_object_location(game, object_get_id(game_get_objects(game, i))) == id_act && space_get_light(game_get_space(game, id_act)) == TRUE)
      {
        if (i == 0)
          obj[i] = (char*)object_get_name (game_get_objects (game, i));
        else if (i == 1)
    ...
    if (id_act != NO_ID)
    {
      ...
      {
        sprintf(str, "  %ld<--| 8D          %2d|", link_getSpace1Id (game_get_link (game, space_get_west(game_get_space(game, id_act)))), (int)id_act);
      }
      else if (space_get_east(game_get_space(game, id_act)) != NO_ID)
      {
        sprintf(str, "      | 8D          %2d|-->%ld", (int)id_act, link_getSpace2Id (game_get_link (game, space_get_east(game_get_space(game, id_act)))));
      }
      else
      {
    ...
    for (i = 0; i < 4; i++)
    {
      ...
      if (game_get_object_location(game, object_get_id(game_get_objects(game, i))) == id_next && space_get_light(game_get_space(game, id_next)))
      {
        if (i == 0)
          obj[i] = (char*)object_get_name (game_get_objects (game, i));
        else if (i == 1)
    ...
    if (id_next != NO_ID)
    {
      ...
      {
        sprintf(str, "  %ld<--|             %2d|", link_getSpace1Id (game_get_link (game, space_get_west(game_get_space(game, id_next)))), (int)id_next);
      }
      else if (space_get_east(game_get_space(game, id_next)) != NO_ID)
      {
        sprintf(str, "      |             %2d|-->%ld", (int)id_next, link_getSpace2Id (game_get_link (game, space_get_east(game_get_space(game, id_next)))));
      }
      else
      {
  ...
  sprintf(str, "     t or take, m or move, d or drop, roll or rl, to or turnon, tf or turnoff, o or open, inspect or i, exit or e");
  screen_area_puts(ge->help, str);
