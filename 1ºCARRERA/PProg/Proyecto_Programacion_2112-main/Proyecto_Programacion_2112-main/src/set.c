...
{ /*Por: Sergio Hidalgo*/
    ...
    for (i = 0; i < set->ids; i++)  /*Bucle en el que se busca la posición del id en la array*/
    {
        if (id == set->array_id[i])
        {
            
            (set->ids)--;   /*Se borra en uno el número de ids*/

            set->array_id[i] = set->array_id[set->ids]; /*Y se sustituye el id*/
...
{ /*Por: Sergio Hidalgo*/
    if ((set->array_id[position]) == NO_ID || !set)
    {
        return NO_ID;
    }
