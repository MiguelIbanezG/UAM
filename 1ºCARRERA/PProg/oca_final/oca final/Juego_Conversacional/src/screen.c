...
/*******************************/
void screen_init(){
  screen_destroy(); /* Se dispone si ya se ha inicializado */
  __data = (char *) malloc(sizeof(char) * TOTAL_DATA);

  if (__data){
    memset(__data, (int) BG_CHAR, TOTAL_DATA); /*Rellena el fondo*/
    *(__data + TOTAL_DATA - 1) = '\0';         /*Cadena de carácteres (termina en NULL)*/
  }
}
...
void screen_paint(){
  ...
  if (__data){
    /* pone(__data); */ /*Pasa data por el terminal*/
    /*Funciona bien si el terminal tiene un tamaño correcto*/
	...
	if (dest[i] == BG_CHAR){
	  ...
	  printf("\033[0;35;47m%c\033[0m", dest[i]); /* fg:black(30);bg:white(47)*/
	}
      }
      printf("\n");
    }
...
/*Reserva de memoria del area de pantalla que se va a usar*/
...
/*Libera la memoria de la anterior función*/
void screen_area_destroy(Area* area){
  if(area)
    free(area);
}

/*Limpia el área de la anterior función*/
...
/*Resetea el cursor en el area de la pantalla usada*/
...
/*Hace que avanze la pantalla y el tablero cambie*/
void screen_area_scroll_up(Area* area){
...
void screen_utils_replaces_special_chars(char* str){
  ...
  /*Reemplaza los carácteres especiales con '??'*/
