/** 
 * @brief Comprueba el funcionamiento del modulo player
 * 
 * @file player_test.c
...
int main(int argc, char** argv) {
  ...
  if (all || test == 28) test1_player_check_inventory();
  if (all || test == 29) test3_player_check_inventory();
  if (all || test == 30) test3_player_check_inventory();
...
void test1_player_check_inventory(){
    Player *p;

    p = player_create(5);
    player_set_addObject(p, 5);
    PRINT_TEST_RESULT(player_check_inventory(p, 5) == TRUE);
    player_destroy(p);
}

void test2_player_check_inventory(){
    Player *p;

    p = player_create(5);
    player_set_addObject(p, 5);
    PRINT_TEST_RESULT(player_check_inventory(p, 1) == FALSE);
    player_destroy(p);
}

void test3_player_check_inventory(){
    Player *p;

    p = player_create(5);
    player_set_addObject(p, 5);
    PRINT_TEST_RESULT(player_check_inventory(p, NO_ID) == FALSE);
    player_destroy(p);
}
