...
void game_callback_turnon(Game *game)
{
  ...
  for (j = 0; j < MAX_SPACES && game->spaces[j] != NULL; j++)
  {
    ...
    if (player_id == space_get_id(game->spaces[j])) /*Bucle que busca el espacio donde está el jugador*/
    {
      break;
    }
  }
  ...
  else
  {
    ...
    space_set_light(game->spaces[j], 1);
    return;
  }
}
...
void game_callback_turnoff(Game *game)
{
  ...
  space_set_light(game->spaces[j], 0);
