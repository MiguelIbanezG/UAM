...
void graphic_engine_paint_game(Graphic_engine *ge, Game *game) /*Esta función pitna el tablero*/
{
  ...
  if ((id_act = game_get_player_location(game)) != NO_ID)
  {
    ...
    if (game_get_object_location(game, object_get_id(game->objects[0])) == id_back)
      obj = '*';
    else
      obj = ' ';
    ...
    if (game_get_object_location(game, object_get_id(game->objects[0])) == id_act)
      obj = '*';
    else
      obj = ' ';
    ...
    if (game_get_object_location(game, object_get_id(game->objects[0])) == id_next)
      obj = '*';
    else
      obj = ' ';
  ...
  if ((obj_loc = game_get_object_location(game, object_get_id(game->objects[0]))) != NO_ID)
  {
    sprintf(str, "  Object location:%d", (int)obj_loc);
    screen_area_puts(ge->descript, str);
  }

  if (die_lastroll(game->die) > 0)
  {
    sprintf(str, "  Ultima tirada:%d", die_lastroll(game->die));
    screen_area_puts(ge->descript, str);
  }
  ...
  sprintf(str, "     next or n, back or b, t or take, d or drop, roll or rl, left or l,            right or r, exit or e");
  screen_area_puts(ge->help, str);
