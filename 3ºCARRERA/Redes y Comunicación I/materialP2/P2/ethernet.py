...
def process_Ethernet_frame(us:ctypes.c_void_p,header:pcap_pkthdr,data:bytes) -> None:
    ...
    Ethernet_dest = data[:6]
    # Ethertype los dos siguientes bytes
    Ethertype = data [12:14]

    # Comprobar si la dirección destino es la propia o la de broadcast.
    if Ethernet_dest != macAddress and Ethernet_dest != broadcastAddr:
        return

    Ethernet_real = struct.unpack('h',Ethertype)

    # Comprobar si existe una función de callback de nivel superior asociada al Ethertype de la trama:
    if not Ethernet_real in upperProtos:
        print("El ethertype: " + str(Ethertype)+ "no se ha encontrado en el diccionario")
        print(upperProtos)
        return
    
    funcion1 = upperProtos[Ethernet_real]

	# En caso de que exista, llamar a la función de nivel superior con los parámetros que corresponde:
    funcion1(us, header, data[14:], Ethernet_orig)
...
def sendEthernetFrame(data:bytes,length:int,etherType:int,dstMac:bytes) -> int:
    ...
    if length > ETH_FRAME_MAX:
        logging.debug('ERROR, la cabezera es más grande de lo permitido.')
        return -1
