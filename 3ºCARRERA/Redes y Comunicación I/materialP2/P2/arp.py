...
def processARPRequest(data:bytes,MAC:bytes)->None:
    ...
    global myIP
    # Extraer la MAC origen contenida en la petición ARP, extramos del byte 0 al 5 que es la direccion MAC de origen
    MAC_orig = data[2:8]
    
    #Si la MAC origen de la trama ARP no es la misma que la recibida del nivel Ethernet retornar
    if MAC_orig != MAC:
        return
    # Extraer la IP origen contenida en la petición ARP, del byte 6 al 10 esta la ip origen
    IP_orig = data[8:12]

    # Extraer la IP destino contenida en la petición ARP, del byte 16 al 20 esta la ip destino
    IP_dest = data[18:22]
    IP_real = struct.pack('!I', myIP)
    print(data)
    print(IP_orig)
    print(IP_real)
    ...
    # Si es la propia IP:
    # Construir una respuesta ARP llamando a createARPReply (descripción más adelante)
    frame = createARPReply(IP_orig, MAC_orig)
    # Enviar la respuesta ARP usando el nivel Ethernet (sendEthernetFrame)
    sendEthernetFrame(frame, len(frame), bytes([0x08,0x06]), MAC_orig)
    return
...
def processARPReply(data:bytes,MAC:bytes)->None:
    ...
    # Extraer la MAC destino contenida en la petición ARP, extraemos la mac destino del bytearray, la que envio la arp request
    MAC_dest = data[18:24]
    ...
    # Copiar la MAC origen a la variable global resolvedMAC, protegemos con lock usando el bloque with
    with globalLock:
    ...
    with cacheLock:
        cache = {requestedIP: MAC_orig}
    # Se debe anidar el par ip/mac, son las de origen porque el arp reply tiene como origen las de destino del arp request
    with globalLock:
    # Cambiar el valor de la variable requestedIP a None
        requestedIP = None
    # Cambiar el valor de la variable awaitingResponse a False
...
def process_arp_frame(us:ctypes.c_void_p,header:pcap_pkthdr,data:bytes,srcMac:bytes) -> None:
    ...
    # Extraer el campo opcode
    if comun != ARPHeader:
        print ("ERROR, la cabecera no es correcta")
    opcode = data[6:8]
...
def initARP(interface:str) -> int:
    ...
    # Realizar una petición ARP gratuita y comprobar si la IP propia ya está asignada
    prueba = ARPResolution(myIP)
    print(prueba)
    if prueba is not None:
    # En caso positivo se debe devolver error
        return -1
...
def ARPResolution(ip:int) -> bytes:
    ...
    # Comprobar si la IP solicitada existe en la caché, protegemos con semaforo
    with cacheLock:
    	if ip in cache:
        # Si está en caché devolver la información de la caché
            return cache[ip]
    ...
    for i in range(3):
        ...
        if awaitingResponse is True:
            ...
            print("Reenvio la petición")
    	# Si se ha recibido respuesta devolver la dirección MAC
        else:
            print("La MAC por la que se pregunta es:")
            return resolvedMAC
