...
def process_UDP_datagram(us,header,data,srcIP):
    ...
    # Loggear (usando logging.debug) los siguientes campos
    logging.debug("Puerto origen:: " + str(int.from_bytes(org_port, byteorder='big')))
    logging.debug("Puerto destino: " + str(int.from_bytes(dest_port, byteorder='big')))
    logging.debug("informacion del paquete: " + str(data[8:].hex()))
    logging.debug("datagrama udp: " + str(data[8:]))
...
def sendUDPDatagram(data,dstPort,dstIP):
    ...
    # El puerto origen lo obtendremos llamando a getUDPSourcePort
    udp_datagram += getUDPSourcePort().to_bytes(2, byteorder='big') 
    udp_datagram += dstPort.to_bytes(2, byteorder='big')
    udp_datagram += int(8+len(data)).to_bytes(2, byteorder="big")
    udp_datagram+= int(0).to_bytes(2, byteorder="big")

    # Añadir los datos
    udp_datagram += data

    # Enviar el datagrama resultante llamando a sendIPDatagram
    sendIPDatagram(dstIP,udp_datagram,17) 
