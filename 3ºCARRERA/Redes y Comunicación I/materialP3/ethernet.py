...
def process_Ethernet_frame(us:ctypes.c_void_p,header:pcap_pkthdr,data:bytes) -> None:
    ...
    ethernet_dest = data[0:6] 
    ethernet_orig = data[6:12] 
    ethertype = data[12:14] 

    # Comprobar si la dirección destino es la propia o la de broadcast.
    if(ethernet_dest == broadcastAddr or ethernet_dest == macAddress): 
        ethernet_real = int.from_bytes(ethertype, byteorder='big')
        if(ethernet_real in upperProtos):
            return upperProtos[ethernet_real](us, header, data[14:], ethernet_orig )
...
def startEthernetLevel(interface:str) -> int:
    ...
    # Obtener y almacenar en una variable global la dirección MAC asociada a la interfaz que se especificaaz
...
def sendEthernetFrame(data:bytes,length:int,etherType:int,dstMac:bytes) -> int:
   ...
   # Si es muy grande se debe devolver error
    if((len(data)+14)>ETH_FRAME_MAX):
        return -1

    # Comprobar los límites de Ethernet. Si la trama es muy pequeña se debe rellenar con 0s mientras que
    if((len(data)+14)<ETH_FRAME_MIN):
            taman = ETH_FRAME_MIN - (len(data)+14)
            data += bytes([0x00]*taman)

    # Construir la trama Ethernet a enviar (incluyendo cabecera + payload). Los campos propios (por ejemplo la dirección Ethernet origen)
    ethernetFrame = bytes()
    ethernetFrame += dstMac
    ethernetFrame += macAddress
    ethernetFrame += etherType.to_bytes(2, byteorder='big')
    ethernetFrame += data

    # Llamar a pcap_inject para enviar la trama y comprobar el retorno de dicha llamada. En caso de que haya error notificarlo
    ret = pcap_inject(handle, ethernetFrame, len(ethernetFrame))

    if(ret!=len(ethernetFrame)):
        return -1
    return 0
