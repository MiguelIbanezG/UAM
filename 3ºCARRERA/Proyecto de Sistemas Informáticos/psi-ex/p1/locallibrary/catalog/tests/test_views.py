...
from catalog.views import author_detail_view, book_detail_view
...
class AuthorCreateViewTest(TestCase):
    def setUp(self):
        ...
        # Give test_user2 permission to renew books (same permisson as adding new authors).
        permission = Permission.objects.get(name='Set book as returned')
        test_user2.user_permissions.add(permission)
        test_user2.save()
        ...
        self.test_author = Author.objects.create(
            first_name='John', last_name='Smith', date_of_birth='2000-12-2', date_of_death='2023-1-1')
    ...
    def test_form_date_of_death_initially_set_to_expected_date(self):
        ...
        response = self.client.post(reverse('author-create'),
                                    {'first_name': self.test_author.first_name,
                                     'last_name': self.test_author.last_name,
                                     'date_of_birth': self.test_author.date_of_birth,
                                     'date_of_death': self.test_author.date_of_death}, follow=True)

        self.assertEqual(response.status_code, 200)

        self.assertIn(b'<strong>Death:</strong> Jan. 1, 2023',
                      response.content)
    ...
    def test_redirects_to_detail_view_on_success(self):
        ...
        # +1 because i am creating one more (the one on the setup has lower id)
        self.assertRedirects(response, reverse(
            'author-detail', kwargs={'pk': (self.test_author.pk + 1)}))

    def test_author_detail_view(self):
        response = author_detail_view('Get', self.test_author.pk)
        self.assertEqual(response.status_code, 200)
...
class BookViewTest(TestCase):
    ...
    def setUpTestData(self):
       # Create a book
        ...
        # Create genre as a post-step
        self.test_book.genre.add(test_genre)
        self.test_book.save()

    def test_book_detail_view(self):
        response = book_detail_view('Get', self.test_book.pk)
        self.assertEqual(response.status_code, 200)
